...
class ManutencaoScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ResourceListScreen(
        ...
        itemTitle: (item) =>
            '${item['numero_os'] ?? item['numero'] ?? 'OS'} · ${item['cliente_nome'] ?? ''}',
        itemSubtitle: (item) =>
            '${item['motivo_chamado'] ?? item['descricao_problema'] ?? ''} · ${item['status'] ?? ''}',
        fab: FloatingActionButton.extended(
            onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                    content: Text(
                        'A criação de OS requer os dados do cliente e do chamado.'))),
